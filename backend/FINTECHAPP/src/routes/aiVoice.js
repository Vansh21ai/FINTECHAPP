...
module.exports = (pool) => {
    ...
    router.post('/ask', async (req, res) => {
        try {
            ...
            const extractionModel = genAI.getGenerativeModel({ model: 'gemini-2.0-flash', safetySettings });
...
Market data: ${marketContext}`;
            ...
            try {
                ...
                if (safeguardError.message.includes('429') || safeguardError.message.toLowerCase().includes('quota') || safeguardError.message.toLowerCase().includes('retry')) {
                    aiResponse = "System Alert: Your Google Gemini API Key has hit a strict rate limit or quota exhaustion. Please wait a minute or swap to a new key in your environment variables.";
                } else if (safeguardError.message.includes('not found') || safeguardError.message.includes('model')) {
                    aiResponse = "System Alert: The targeted AI Model does not exist in your Gemini dashboard's geographic region.";
                } else {
